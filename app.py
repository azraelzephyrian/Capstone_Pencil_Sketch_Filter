...
def resize_image():
    ...
    try:
        scale_percent = float(request.form.get("scale_percent", 100))
        if scale_percent <= 0:
            return "Scale percent must be positive", 400
...
def call_llm_with_prompt(transcript: str) -> dict:
    system_prompt = system_prompt = """
        You will be given a natural language voice command for an image editing app. Respond ONLY with a JSON object.

        Your response **must include**:
        - "action": the action being requested (in ALL CAPS)
        - "params": an object with:
            - "known": key-value pairs for any parameters the user *already mentioned*
            - "missing": a list of parameter names needed for the action but not mentioned

        Examples:

        Input: "resize to 70 percent"
        → Output: {
        "action": "APPLY RESIZE",
        "params": {
            "known": { "scale_percent": "70" },
            "missing": []
        }
        }

        Input: "resize the image"
        → Output: {
        "action": "APPLY RESIZE",
        "params": {
            "known": {},
            "missing": ["scale_percent"]
        }
        }

        Input: "apply cel shade with 10 colors"
        → Output: {
        "action": "APPLY CEL SHADE",
        "params": {
            "known": { "num_colors": "10" },
            "missing": ["bilateral_d", "bilateral_sigmaColor", "bilateral_sigmaSpace", "median_blur_ksize", "canny_thresh1", "canny_thresh2", "edge_thickness"]
        }
        }

        If the command is too ambiguous, return:
        {
        "action": "UNKNOWN",
        "params": {
            "known": {},
        ...
        Below is a list of actions.  If a user asks a specific question about HOW to do something (like "how do I apply a mosaic filter"), 
        you are to choose the EXPLAIN action; these queries should be interpreted not as requests to perform an action, but to explain it.  Otherwise, refer to the list below.    
        List of available actions:
        1. ROTATE LEFT — if the user wants to rotate the image counter-clockwise
        2. ROTATE RIGHT — if the user wants to rotate the image clockwise
        3. APPLY PENCIL — if the user wants to apply the basic pencil sketch
        4. APPLY GAN PENCIL — if the user wants to generate a pencil sketch using GAN
        5. APPLY GAN EDGE — if the user wants to generate an edge sketch using GAN
        6. APPLY CEL SHADE — if the user wants to apply cel-shading. The words “cel”, “cell”, or “sell” should all be interpreted the same in this context
        7. APPLY POSTER — if the user wants to apply the posterization filter
        8. APPLY MOSAIC — if the user wants to apply the Voronoi mosaic filter
        9. APPLY STROKE — if the user wants to apply the tapered pencil strokes
        10. APPLY GAUSSIAN — if the user wants to apply the Gaussian sketch
        11. APPLY COLORIZATION — if the user wants to colorize the image using another image
        12. APPLY RESIZE — if the user wants to resize or scale the image
        13. DOWNLOAD — if the user wants to download the image
        14. UNKNOWN — if the command doesn't match any of the above
        15. HELP -- If the user asks for help, a list of commands, asks what they can say, or how they can use something
        16. EXPLAIN — if the user asks how to do something in the app (e.g., "how do I change the color count")

        If the user is asking a question about how to use the app (e.g., "how do I change the color count"), respond with:

        {
        "action": "EXPLAIN",
        "question": "..."  ← repeat the question as parsed
        }

        This indicates they want a tutorial-style response based on their query.

        Notes:
        - Be robust to voice recognition errors like "sell shade" → "cel-shade".
        - Ignore filler words like "please", "can you", "I want to", etc.
        - DO NOT return more than one response.
        - DO NOT include explanations, punctuation, or formatting.
        NEVER include explanations or any text outside the JSON block.
        """


    user_prompt = f"Voice Command: {transcript.strip()}"
    ...
    try:
        ...
        return {"action": "UNKNOWN", "param": None, "value": None}




@app.route("/interpret_voice_command", methods=["POST"])
def interpret_voice_command():
    ...
    try:
        intent = call_llm_with_prompt(transcript)
        print(f"✅ Intent from LLM: {intent}")
...
def explain_query():
    ...
    print(f"🧠 Coaching request: {question}")

    system_prompt = """
        You are the in-app voice tutor for a web-based image editor. The user has asked a question.
        Your job is to coach them with clear, concise voice instructions.

        Return your answer as a JSON list of voice narration lines — each one should be a short spoken sentence (no more than 20 words) for use with speech synthesis.

        Do not include explanations or disclaimers. Just the JSON array of narration lines.
    ...
    try:
        ...
        print("📚 Raw explanation content:", raw_content)
        tutorial_lines = json.loads(raw_content)
        return jsonify({"lines": tutorial_lines}), 200
    except Exception as e:
